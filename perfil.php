...
if (!$is_logged_in || $user['role'] != 'usuario') {
...
try {
    $stmtInfo = $conn->prepare("SELECT first_name, last_name, email, phone, dni FROM usuarios WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($phone)) {
        ...
        try {
            $stmt = $conn->prepare("UPDATE usuarios SET first_name = ?, last_name = ?, email = ?, phone = ? WHERE id = ?");
